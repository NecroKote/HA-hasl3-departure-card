...
import {
    ...
    type EntityInfoAction,
    type ServiceCallAction,
} from "./DepartureCard.config"
...
export class HASLDepartureCard extends LitElement implements LovelaceCard {
    ...
    static async getConfigElement() {
        return await import("../DepartureCardEditor").then(() =>
            document.createElement("hasl4-departure-card-editor"),
        )
    }
    ...
    render() {
        ...
        const renderLastUpdated = this.isManyEntitiesSet()
            ? () => nothing
            : () => {
    ...
    private getFirstEntity():
        | [HassEntity, DepartureAttributes]
        | [undefined, undefined] {
            ...
            (
                attrs.departures
                    ...
                    .filter((d) => {
                        ...
                        return (
                            diff + (this.config?.show_departed_offeset ?? 0) >=
                            0
                        )
                    }) || []
    ...
    private getDeparturesCombined(entities: string[]) {
        ...
        return (
            entities
                ...
                .filter(
                    (attrs): attrs is DepartureAttributes =>
                        attrs !== undefined,
                )
        ...
        return html`
            <div class="departures">
                ...
                ${departures.map((dep) => {
                    ...
                    const mostImportantDeviation = dep.deviations?.sort(
                        (a, b) => b.importance_level - a.importance_level,
                    )?.[0]

                    const departureTime = this.config?.show_time_always
                        ? expectedAt.toLocaleTimeString(lang, {
                              hour: "numeric",
                              minute: "numeric",
                          })
                        : (() => {
                    ...
                    const lineIconClass = this.lineIconClass(
...
const isEntityInfoAction = (a: ClickAction): a is EntityInfoAction =>
    "entityId" in (a as object)
const isServiceCallAction = (a: ClickAction): a is ServiceCallAction =>
    "service" in (a as object)

function isDepartureAttrs(
    item: Record<string, unknown>,
): item is DepartureAttributes {
